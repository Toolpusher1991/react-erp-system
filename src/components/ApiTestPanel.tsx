...
import { testConnection, checkHealth, getUsers, isServerOnline } from '../services/api';
  ...
  const performConnectionTest = async () => {
    ...
    try {
      ...
      if (usersResult.error) {
        ...
        addTestResult(`✅ Users loaded: ${usersResult.data?.length || 0} users found`);
      }
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        marginBottom: '1rem',
        padding: '0.5rem',
        background: status.isOnline ? 'rgba(16, 185, 129, 0.2)' : 'rgba(239, 68, 68, 0.2)',
        borderRadius: '6px',
        border: `1px solid ${status.isOnline ? 'rgba(16, 185, 129, 0.4)' : 'rgba(239, 68, 68, 0.4)'}`
      }}>
        ...
        <div>
          <div style={{ fontWeight: 'bold' }}>
            {status.loading ? 'Testing...' : status.isOnline ? 'Connected' : 'Disconnected'}
          </div>
          {status.serverInfo && (
      ...
      {status.serverInfo && (
        <div style={{ marginBottom: '1rem' }}>
          <div style={{ fontWeight: 'bold', color: '#34d399', marginBottom: '0.25rem' }}>
            📊 Server Info:
          </div>
          <div style={{ fontSize: '0.75rem', color: '#d1d5db' }}>
            Environment: {status.serverInfo.environment}<br/>
            Status: {status.serverInfo.status}<br/>
            Timestamp: {new Date(status.serverInfo.timestamp).toLocaleTimeString()}
          </div>
        </div>
      )}
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <div style={{ fontSize: '0.75rem', color: '#d1d5db' }}>
            {status.users.map((user: any, idx: number) => (
              <div key={idx}>{user.name} ({user.role})</div>
            ))}
          </div>
        </div>
      )}
      ...
      {/* Test Results Log */}
      <div>
        <div style={{ fontWeight: 'bold', color: '#a78bfa', marginBottom: '0.5rem' }}>
          📝 Test Log:
        </div>
        <div style={{ 
          background: '#111827',
          ...
          {testResults.length === 0 ? (
            ...
            testResults.map((result, idx) => (
              <div key={idx} style={{ 
                ...
                color: result.includes('❌') ? '#f87171' : 
                       result.includes('✅') ? '#34d399' :
                       result.includes('⚠️') ? '#fbbf24' : '#d1d5db'
              }}>
                {result}
              </div>
            ))
